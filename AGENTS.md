...
- 协作画布中的 Dropdown 等 portal 弹层要标记 `data-canvas-no-zoom`，节点多选处理也要排除弹层目标；否则菜单点击会被画布平移截获。右键菜单验收必须实际点击菜单项并验证下载完成，不能只检查菜单显示。
- 画布拖动和框选使用 `preventDefault` 时，点击空白处或节点标题需退出输入框焦点并清除残留文字选区；验收要覆盖全选文字后切回节点复制粘贴，同时保留输入框原生快捷键。
- 浏览器验收应等待实际接口响应和控件状态变化；带 loading 图标的 Ant Design 操作按钮使用稳定的 `aria-label`，避免图标残留改变无障碍名称后误判请求超时。展开弹窗内容后先等待控件可见，再检查布局或截图。
- 弹窗内画笔、裁剪等依赖坐标的浏览器验收，必须等弹窗有限动画结束后再读取几何位置；截图也要等待动画完成，控件可见本身不能保证坐标已稳定。
- 协作 multipart 请求调整后必须覆盖带附件、仅普通字段、空表单和中途取消，核对完整边界、长度与临时密文清理；使用支持异步迭代的编码库，不能凭带文件场景通过就认为纯字段表单也能完成。

- 服务器协作入口 `/collaboration` 已上线；“项目尚未上线、无需兼容旧数据”的规则不适用于协作主库。升级前先备份并验证恢复，保留本机数据库、主密钥和既有云盘挂载，数据库迁移后核对原记录，不能让旧服务直接打开新版本数据库。
- 协作服务器宿主 Python 为 3.6，不得假定有新版 pathlib 或 SQLite backup API；数据库快照使用项目 Node / SQLite 工具。隔离验证要使用独立副本，WAL 数据库需连同 WAL 保存，不能只复制主文件后宣称数据完整。
- 修改协作 Nginx 配置后，重建 gateway 并核对宿主与容器内配置哈希，再验证实际 HTTPS 路由；单文件 bind mount 可能保留旧 inode，不能仅凭 reload 成功判定配置已生效。

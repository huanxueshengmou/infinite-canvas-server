...
+ [新增] 协作画布可导出含附件的单个离线 HTML，并导入为可继续编辑的新画布，保留节点、连线、分组与白板且排除私有配置。

+ [新增] 协作画布支持图片画笔、箭头、文字与裁剪编辑，完成后覆盖或复制到剪贴板；增加可整体移动的画布组和可缩放书写、无出入端口的白板节点。
+ [新增] 协作画布支持多文件拖入与复制粘贴、Markdown 编辑预览、图片视频附件右键下载，以及双击空白处选择节点类型和模板；不支持预览的文件保留下载入口。
+ [新增] 协作画布支持撤销重做、框选多选、批量移动删除恢复、拖到节点连线与右键断线，并保护协作者的后续修改。
+ [新增] 隐私请求支持多种 HTTP 方法、JSON / 表单 / multipart / 文本与 XML、自定义请求头和认证方式，域名白名单开关与黑名单独立保存。
+ [修复] 服务关闭时先断开在线协作连接，避免升级切换被 WebSocket 阻塞。

+ [修复] 协作升级后拒绝旧页面的节点写入并提示刷新，防止旧配置覆盖新工作流字段。
+ [新增] 协作画布支持具名鼠标、同步连线、自定义文本与 JSON 节点，以及可复用的生图、视频和隐私 API 模板与明确授权的结果发布。
+ [调整] 协作文件默认上限提高至 500 MiB，管理员可直接调整，并采用完整性验证后的流式下载控制内存占用。
+ [新增] 协作登录页支持直接自助注册并自动登录，新账号仅能访问自己创建或获分享授权的画布。
+ [新增] 提供带账户与分享鉴权的异步多人协作服务，支持隐私 API 节点、加密文件与云盘备份恢复，并完成 20 人并发编辑验证。
+ [修复] 模型脚本编辑弹窗使用 Ant Design 6 的 container 样式字段，恢复前端类型检查。

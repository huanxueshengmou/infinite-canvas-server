...
本分支提供服务器协作入口 `/collaboration`：支持自助注册、异步多人同步、带用户名的协作者鼠标、节点连线、自定义文本 / JSON 节点，以及可保存复用的生图、视频和 API 请求模板。文件默认上限 **500 MiB**，管理员可在「服务管理」直接调整。

图片、视频和文件可批量拖入或通过 Ctrl/⌘+V 粘贴，选中公共节点可复制粘贴；图片、视频和附件右键即可下载。双击空白处选择节点类型或模板，支持 Markdown 编辑、预览和 `.md` 文件导入，不支持预览的格式作为下载附件保留。

右键图片可用画笔、箭头、文字和框选裁剪编辑，完成后选择覆盖原图或复制图片到剪贴板。公开节点可编组并一起拖动；白板支持手写、文字和自由调整大小，没有输入或输出端口。

进入画布后点击「节点模板」创建节点，将右侧输出圆点拖到下游节点即可连线，也可指定输入端口；右键连线直接断开，点击线可重接。支持 Ctrl/⌘+Z 撤销、Ctrl+Y 或 Ctrl/⌘+Shift+Z 重做、Ctrl/⌘+拖拽框选和点击多选，以及批量移动、删除与恢复。

生图和视频模板参考本地 ComfyUI 的 API 节点，支持 JSON、URL 编码表单、multipart 文件表单和原始文本 / XML，以及自定义请求头与认证方式。密钥与结果仅创建者可见，需要共享时先预览并明确发布；视频任务提交后可单独查询状态。「服务管理」提供域名白名单开关：开启使用白名单，关闭使用黑名单，两份名单独立保存。

分享支持只读 / 编辑、口令、到期与撤销。文件和加密数据库备份写入指定云盘，事务数据库保留在本机。原本地画布位于 `/canvas`；部署与隐私边界见 [协作服务文档](server/README.md)，节点用法见 [协作节点与模板](server/WORKFLOWS.md)，测试范围与实测数据见 [验收记录](server/VALIDATION.md)。

升级后请刷新已打开的协作页面；旧页面的节点写入会被拒绝，避免覆盖新节点配置。

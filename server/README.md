...
7. 从节点右侧输出圆点拖到下游左侧输入圆点连线，也可依次点击端口。点击线可重连或断开；节点删除会一起清理关联连线。详细参数和示例见 [协作节点与模板](WORKFLOWS.md)。
...
- 协作协议版本为 `2`，与数据库版本独立。节点修改和隐私保存 / 执行 / 发布要求 `X-Canvas-Protocol: 2`，WebSocket 使用 `events?v=2`。缺少或不匹配版本的写入返回 409 且不保存，旧连接以 4001 关闭并提示刷新；升级后请重新加载页面。
...
API 请求和结果媒体获取仅允许管理员批准的 HTTPS 443 域名。每次请求检查全部 DNS 解析结果，拒绝内网、回环、保留、链路本地和云元数据地址，并将通过校验的地址固定到实际 TLS 连接；不跟随重定向。响应中的当前 API 密钥会脱敏。获取媒体时不向媒体主机转发 API 密钥、会话 Cookie 或 Authorization。
...
- 数据库版本 2 增加连线和个人模板表。已有版本 1 会先在本地创建独立的 `canvas.sqlite.before-v2-UUID.sqlite` 备份，再进行增量迁移，保留账户、会话、节点和文件引用；未知版本拒绝启动。回退旧服务时需在维护窗口恢复对应版本 1 备份，不能让旧程序直接打开版本 2 数据库。
